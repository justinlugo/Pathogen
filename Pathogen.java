...
// For this assignment, we were told to modify our professor's code, so that as well as his comments
// are below, modified per the assignment details to pass all test cases.
...
public class Pathogen
{
	...
	public static void enableAnimation() { Pathogen.animationEnabled = false; }
	...
	private static final char COVID 		 = '*';	 // COVID

	// HashSet that we'll be using to store our paths.
	public static HashSet<String> validPaths = new HashSet<String>();

	public static HashSet<String> findPaths(char [][] maze)
	{
		// Function to find paths, pretty much just returns the paths after we solve the maze.
		solveMaze(maze);
		return Pathogen.validPaths;
	}
	...
	{
		// If there are no valid paths, we return an empty string.
		String validPath = "";
		...
		// Let's gooooooooo!!
		return solveMaze(maze, visited, startRow, startCol, height, width, validPath);
	}

	private static boolean solveMaze(char [][] maze, char [][] visited,
	                                 int currentRow, int currentCol,
	                                 int height, int width, String validPath)
	{
		...
		{
			// Add our path to the HashSet, shortening the string by 1 to get rid of the excess space
			// added on in fasterConcatenation().
			Pathogen.validPaths.add(validPath.substring(0, validPath.length() - 1));
			if (Pathogen.animationEnabled)
			{
				char [] widgets = {'|', '/', '-', '\\', '|', '/', '-', '\\',
				                   '|', '/', '-', '\\', '|', '/', '-', '\\', '|'};
		...
		for (int i = 0; i < moves.length; i++)
		{
			...
			// 3b) Change state.
			// Before moving the person forward in the maze, we
			...
			validPath = fasterConcatenation(validPath, i);
			...
			solveMaze(maze, visited, newRow, newCol, height, width, validPath);

			// 3e) Undo state change.
			// Note that if we return from the previous call,
			...
			validPath = validPath.substring(0, validPath.length() - 2);
			maze[currentRow][currentCol] = PERSON;
	...
	{
		// Modified to add that if the maze has covid and bounds to keep the person within the maze.
		if (maze[row][col] == WALL || visited[row][col] == BREADCRUMB || maze[row][col] == COVID ||
				row <= 0 || row > height - 2 || col <= 0 || col > width - 2)
				{
					return false;
				}
		return true;
	...
	public static String fasterConcatenation(String validPath, int i)
	{
		// This is our function that builds our string by appending the letters corresponding to the
		// directional integer passed in.
		StringBuilder builder = new StringBuilder();

		// Moving Left.
		if (i == 0)
		{
			builder.append(validPath).append("l ");
		}

		// Moving Right.
		if (i == 1)
		{
			builder.append(validPath).append("r ");
		}

		// Moving Up.
		if (i == 2)
		{
			builder.append(validPath).append("u ");
		}

		// Moving Down.
		if (i == 3)
		{
			builder.append(validPath).append("d ");
		}
...
/*
	public static void main(String [] args) throws IOException
	{
		...
		char [][] maze = readMaze("itwasriggedfromthestart.txt");
